...
class SphinxClient
{
	...
	function SphinxClient ()
	{
		...
		$this->_mbenc		= "";
		$this->_arrayresult	= true;     //modified by void
		$this->_timeout		= 0;
	}
	...
	function Where($rule)
	{
		$limit  = isset($rule['limit']) && $rule['limit']
				  ? intval($rule['limit'])
			      : 10;
		...
		if ( isset($rule['eq']) && is_array($rule['eq']) && !empty($rule['eq']) ) {
			foreach ($rule['eq'] as $key => $value) {
				...
				$this->SetFilter($key, array($value) );
			}
		}
		$this->SetFilter('removed', array(0) );
			...
			foreach ($rule['scope'] as $key => $value) {
				if ( is_array($value) && count($value) == 2 && $value[0] < $value[1] ) {
					if ( is_int($value[0]) && is_int($value[1]) ) {
						$this->SetFilterRange($key, intval($value[0]), intval($value[1]) );
					} else {
						$this->SetFilterFloatRange($key, floatval($value[0]), floatval($value[1]) );
					}
				}
			}
		}
		...
		//Ö÷¼üid·¶Î§²éÑ¯
		if ( isset($rule['range']) && is_array($rule['range']) && count($rule['range']) == 2 ) {
			$min = intval($rule['range'][0]);
			$max = intval($rule['range'][1]);
			if ( $min <= $max ) {
	...
	function find($index, $rule)
	{
		$orderBy = array();

		$group   = isset($rule['group']) ? $rule['group'] : array();

		foreach($group as $key => $value) {
			$this->SetGroupBy($key, SPH_GROUPBY_ATTR, "@group $value" );
		}

		$order   = isset($rule['order']) ? $rule['order'] : array();
		foreach ($order as $key => $value) {
			$orderBy[] = isset($rule['pk']) && $rule['pk'] == $key ? "@id $value" : "$key $value";
		}
		if ( !empty($orderBy) ) {
			$this->SetSortMode(SPH_SORT_EXTENDED, implode(',', $orderBy) );
		}
		...
		if ( isset($rule['like']) && is_array($rule['like']) && !empty($rule['like']) ) {
			...
			foreach ($rule['like'] as $key => $val) {
				...
				if ( is_array($val) ) {
					foreach ($val as $v) {
						if ( trim($v) != '' ) $values[] = $this->split($v, $index);
					}
				} else {
					if ( trim($value) != '' ) $values[] = $this->split($val, $index);
				}
			}

			if ( !empty($keys) && !empty($values) ) {
				$keyWord = "@(".implode(",", $keys).") (".implode(" | ", $values).")";
			}
		}
		
		$data = $this->Search($keyWord, $index);
		if (isset($rule['exclude']) && is_array($rule['exclude']) && !empty($rule['exclude']) ) {
			foreach($data['list'] as $k => $row) {
				if ( in_array($row['id'], $rule['exclude']) ) {
					unset($data['list'][$k]);
				}
			}
		}
	...
	function split($txt, $index)
	{
		if ( empty($index) ) return '';

		$words   = array();
		$indexes = explode(",", $index);
		foreach ($indexes as $vIndex) {
			$list = $this->BuildKeyWords($txt, $vIndex, true );
			if ( !is_array($list) ) continue;
			
			foreach ($list as $data) {
				
				if ( isset($data['docs']) && $data['docs'] > 0 && isset($data['tokenized']) ) {
					$words[] = $data['tokenized'];
				}
			}			
		}
		if ( empty($words) ) return $txt;
	...
	{
		 $result = $this->Query($keyWord, $index);
	...
	function getArray($sphinxResult)
	{
		$list = array();
		...
		return array(
			'list'  => $list,
			'total' => isset($sphinxResult['total_found']) ? $sphinxResult['total_found'] : 0,
		);
	}

	function getOffset($pageRows = 15, $page = 0)
	{
